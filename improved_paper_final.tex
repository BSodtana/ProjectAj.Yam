...
The pipeline has five stages: (1) dataset loading via TDC; (2) cold-drug split generation with leakage checks; (3) molecular graph and fingerprint construction from SMILES using RDKit; (4) Siamese pair modeling with logit adjustment and deferred re-weighting; and (5) multi-metric evaluation on held-out folds. Model selection is driven by validation macro PR-AUC, chosen because it is sensitive to tail-class retrieval under imbalance---unlike accuracy or ROC-AUC, which are dominated by frequent classes.

\section{Dataset Preparation and Cold-Drug Split Design}

The DrugBank DDI benchmark is loaded through TDC and standardized to three fields: \texttt{drug\_a\_smiles}, \texttt{drug\_b\_smiles}, and integer label \texttt{y}. The split procedure normalizes all pairs to unordered form, removes conflicting assignments (pairs labeled with multiple interaction types), and uses degree-aware balancing to assign each drug to exactly one fold, with explicit graph-based leakage checks. The preprocessing summary is:
\begin{itemize}
...
\end{itemize}
The selected configuration uses protocol S1 with $k=5$ folds and seed 42. Protocol S1 enforces strict cold-drug separation: every drug is assigned to exactly one fold, and all pairs containing a fold-$i$ drug are placed in fold $i$, preventing any test drug from appearing in training under any pair context. Split statistics are shown in Table~\ref{tab:split_stats}.
...
On average, 81.8 of 86 classes appear in each test fold. Variation in class coverage explains part of the fold-wise variation in tail-class metrics: a fold with more classes present in test data will naturally yield evaluable scores for more minority classes.

\section{Molecular Graph and Fingerprint Features}

Each drug is parsed from SMILES into a molecular graph using RDKit. Node features (7 dimensions) encode atomic number, degree, aromaticity flag, formal charge, hybridization code, ring membership flag, and total hydrogen count. Edge features (5 dimensions) encode bond type, conjugation flag, aromaticity flag, ring membership flag, and stereo code. The ECFP4 fingerprint (radius 2, 2048 bits) is projected through a learnable linear layer, batch-normalized, passed through ELU, and dropped with probability 0.2 before fusion with the graph embedding. This projection allows the model to weight fingerprint regions by task relevance rather than using the bit vector passively.

\section{Siamese Pair Model}

The architecture uses a Siamese encoder: both drugs share the same graph attention backbone, enforcing permutation symmetry---swapping $d_a$ and $d_b$ should not alter the interaction prediction. Each drug passes through three GAT layers (4 attention heads, hidden dimension 64), with ELU activation and dropout (0.2) between layers, followed by global mean pooling to produce a graph-level embedding $\mathbf{g} \in \mathbb{R}^{128}$. The projected fingerprint vector $\mathbf{p} \in \mathbb{R}^{128}$ is concatenated with $\mathbf{g}$ and passed through a fusion MLP to yield the final drug embedding:
\begin{equation}
\mathbf{h}_a = \phi([\mathbf{g}_a \| \mathbf{p}_a]), \qquad \mathbf{h}_b = \phi([\mathbf{g}_b \| \mathbf{p}_b]),
\end{equation}
where $\phi$ denotes the fusion MLP and $\|$ denotes concatenation. The pair representation combines four complementary views:
\begin{equation}
\mathbf{z}_{ab} = [\mathbf{h}_a \| \mathbf{h}_b \| |\mathbf{h}_a - \mathbf{h}_b| \| \mathbf{h}_a \odot \mathbf{h}_b].
\end{equation}
Concatenation $\mathbf{h}_a \| \mathbf{h}_b$ captures individual drug properties; absolute difference $|\mathbf{h}_a - \mathbf{h}_b|$ captures structural dissimilarity; element-wise product $\mathbf{h}_a \odot \mathbf{h}_b$ captures multiplicative co-activation patterns. With $\mathbf{h}_a, \mathbf{h}_b \in \mathbb{R}^{128}$, the pair feature vector has dimension 512. A classifier head with a single hidden layer of 256 units (ELU, dropout 0.2) maps this to 86 output logits.

\section{Long-Tail Objective}

Class imbalance is addressed through two mechanisms. Logit adjustment shifts the decision boundary by adding scaled class log-priors to raw logits:
\begin{equation}
...
Evaluation includes accuracy, macro- and micro-F1, Cohen's kappa, macro ROC-AUC, macro PR-AUC, Expected Calibration Error (ECE), Brier score, and tail macro PR-AUC (computed over the bottom 20\% of classes by training support). Macro PR-AUC is the selection metric because it penalizes both false positives against the large negative pool and false negatives in small-support classes, making it the most informative metric under imbalance.
